...
router.post("/", async (request, response) => {
  try {
    const { name, address, city, phone, email, facilities, price, roomTypes } = request.body;

    // Check for required fields
    if (!name || !address || !city || !price || !roomTypes) {
      return response.status(400).send({
        message: "Please send all required fields: name, address, city, price, roomTypes",
      });
    }

    // Validate phone (optional, if provided)
    if (phone && !/^[0-9]{10}$/.test(phone)) {
      return response.status(400).send({ message: "Invalid phone number format. It should be 10 digits." });
    }
...
router.get("/hotels/:attractionId", async (req, res) => {
  try {
    ...
    if (hotels.length === 0) {
      return res.status(404).json({ message: "No hotels found for this attraction location" });
    }
